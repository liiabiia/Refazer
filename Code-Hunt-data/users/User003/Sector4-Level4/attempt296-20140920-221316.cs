    ...
    public static int Puzzle(int[] a) {
		...
		if(list.Count > 0){
			if(list[i]%2!=0){
				...
				{
					if(list[i+1]%2!=0 && list[i+1]>list[i] && (list[i]<0&&list[i+1]<0))i++;
					else break;
				}
				return list[i];
			}
				...
				while(i<list.Count-1)
				{
					...
					else if(!isPower(list[i],2)&&isPower(list[i+1],2))break;
					else if(isPower(list[i],2)&&!isPower(list[i+1],2))i++;
					else if(list[i+1]%2==1 && list[i]<list[i+1])i++;
					else break;
				}
				return list[i];
			}
